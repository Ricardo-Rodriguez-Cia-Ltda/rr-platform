...
|---|---|---|---|
| `apps/pricing-api` | La API de precios | `GET /search`, `/product`, `/price`, `/mejor-precio`, `/facetas`, `POST /credito/mock`, y las mismas por proveedor bajo `/api/<proveedor>/...` | Vercel (proyecto `captador-precios-proveedores`) y, en paralelo, el PC de oficina vía `npm run serve` + Cloudflare Tunnel — ver [Hosting local](#hosting-local-pc-oficina--cloudflare-tunnel) |
